  ...
  return (
    <section id="cinematic-fiber-optic-stage" className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-[var(--sdep-border)] pb-5">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--sdep-teal)]">Republic of Kenya &middot; Inter-ministerial data exchange</p>
          <h2 className="mt-2 text-2xl font-semibold tracking-tight text-[var(--sdep-text)] sm:text-3xl">Sovereign verification desk</h2>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-[var(--sdep-text-muted)]">A controlled simulation of a government request answered by a protected ministry vault. The result is a signed decision, not a copy of the citizen record.</p>
        </div>
        <div className="rounded-lg border border-[var(--sdep-border)] bg-[rgba(16,33,42,0.64)] px-3 py-2 text-right"><p className="text-[10px] font-semibold uppercase tracking-wider text-[var(--sdep-text-subtle)]">Record class</p><p className="mt-1 text-xs font-semibold text-[var(--sdep-text)]">Controlled demonstration</p><p className="mt-1 text-[10px] text-[var(--sdep-text-muted)]">SDEP-ZKP-v2 &middot; EAT</p></div>
      </div>

      <UseCaseSelector activeUseCaseId={useCase.id} onSelectUseCase={onSelectUseCase} />

      <div className="overflow-hidden rounded-xl border border-[var(--sdep-border)] bg-[rgba(12,23,32,0.86)] shadow-2xl shadow-black/20">
        <div className="border-b border-[var(--sdep-border)] bg-[rgba(20,43,53,0.48)] px-4 py-4 sm:px-6">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[var(--sdep-text-subtle)]">Simulation route</p><p className="mt-1 text-sm font-semibold text-[var(--sdep-text)]">Requesting authority to protected source to signed response</p></div><div className="flex items-center gap-2 text-xs text-[var(--sdep-text-muted)]"><span className={`h-2 w-2 rounded-full ${isExecuting ? 'animate-pulse bg-[var(--sdep-warning)]' : 'bg-[var(--sdep-success)]'}`} /></div></div>
          <div className="mt-4 grid gap-2 md:grid-cols-[1fr_auto_1fr_auto_1fr] md:items-stretch">
            <RouteCard label="Requesting authority" title={useCase.requestingMinistry} detail="Asks one purpose-limited question" tone="blue" />
            <RouteArrow />
            <RouteCard label="Protected source authority" title={useCase.targetVault} detail="Evaluates the private record internally" tone="teal" />
            <RouteArrow />
            <RouteCard label="Returned to requester" title="Signed decision + proof" detail="No raw protected record transferred" tone="green" />
          </div>
        </div>
...
function RouteCard({ label, title, detail, tone }: { label: string; title: string; detail: string; tone: 'blue' | 'teal' | 'green' }) {
  const color = tone === 'blue' ? 'text-[var(--sdep-blue)]' : tone === 'green' ? 'text-[var(--sdep-success)]' : 'text-[var(--sdep-teal)]';
  return <div className="rounded-lg border border-[var(--sdep-border)] bg-[rgba(7,16,24,0.72)] p-3"><p className={`text-[10px] font-semibold uppercase tracking-wider ${color}`}>{label}</p><p className="mt-2 text-sm font-semibold text-[var(--sdep-text)]">{title}</p><p className="mt-1 text-[11px] text-[var(--sdep-text-muted)]">{detail}</p></div>;
}

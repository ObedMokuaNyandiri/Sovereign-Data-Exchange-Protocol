...
function RouteCard({ label, title, detail, tone }: { label: string; title: string; detail?: string; tone: 'blue' | 'teal' | 'green' }) {
  const color = tone === 'blue' ? 'text-[var(--sdep-blue)]' : tone === 'green' ? 'text-[var(--sdep-success)]' : 'text-[var(--sdep-teal)]';
  return <div className="rounded-lg border border-[var(--sdep-border)] bg-[rgba(7,16,24,0.72)] p-3"><p className={`text-[10px] font-semibold uppercase tracking-wider ${color}`}>{label}</p><p className="mt-2 text-sm font-semibold text-[var(--sdep-text)]">{title}</p>{detail && <p className="mt-1 text-[11px] text-[var(--sdep-text-muted)]">{detail}</p>}</div>;
}

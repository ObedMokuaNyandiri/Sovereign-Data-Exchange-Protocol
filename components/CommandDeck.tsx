    ...
    <section id="sdep-command-deck" className="overflow-hidden rounded-2xl border border-[var(--sdep-border)] bg-[rgba(12,23,32,0.84)] shadow-xl shadow-black/10">
      <div className="border-b border-[var(--sdep-border)] px-4 py-4 sm:px-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <span className="rounded-full border border-[rgba(16,185,129,0.3)] bg-[rgba(16,185,129,0.08)] px-2.5 py-1 text-[11px] font-semibold text-[var(--sdep-success)]">SDEP-ZKP v2 active</span>
        </div>
...
function BriefingPanel({ onOpenPitch }: { onOpenPitch: () => void }) {
  return <div className="space-y-5"><div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-start"><div><h3 className="text-2xl font-semibold text-[var(--sdep-text)]">Verification without exposure.</h3><div className="mt-5 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-[var(--sdep-border)] bg-[rgba(11,17,32,0.44)] p-3"><strong className="text-sm text-[var(--sdep-success)]">0 bytes</strong><p className="mt-1 text-xs text-[var(--sdep-text-muted)]">Protected records exposed</p></div><div className="rounded-xl border border-[var(--sdep-border)] bg-[rgba(11,17,32,0.44)] p-3"><strong className="text-sm text-[var(--sdep-blue)]">Signed</strong><p className="mt-1 text-xs text-[var(--sdep-text-muted)]">Proof returned with every answer</p></div><div className="rounded-xl border border-[var(--sdep-border)] bg-[rgba(11,17,32,0.44)] p-3"><strong className="text-sm text-[var(--sdep-warning)]">Audited</strong><p className="mt-1 text-xs text-[var(--sdep-text-muted)]">Exchange recorded for review</p></div></div></div><button type="button" onClick={onOpenPitch} className="rounded-lg bg-[var(--sdep-teal)] px-4 py-2.5 text-xs font-semibold text-[#f8fafc] hover:bg-[#7dd3fc]">Open full briefing</button></div></div>;
}
    ...
    <section id="sdep-command-deck" className="overflow-hidden rounded-2xl border border-[var(--sdep-border)] bg-[rgba(12,23,32,0.84)] shadow-xl shadow-black/10">
      <div className="border-b border-[var(--sdep-border)] px-4 py-4 sm:px-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--sdep-teal)]">Evidence and administration</p><h2 className="mt-1 text-xl font-semibold text-[var(--sdep-text)]">Inspect the exchange</h2><p className="mt-1 text-xs text-[var(--sdep-text-muted)]">Review signed results, protection controls, and the technical implementation behind each decision.</p></div>
...
function ExchangeMatrix({ activeUseCase, onSelectUseCase }: { activeUseCase: UseCaseConfig; onSelectUseCase: (uc: UseCaseConfig) => void }) {
  return <div className="space-y-4"><div><h3 className="text-base font-semibold text-[var(--sdep-text)]">Inter-ministerial exchange matrix</h3><p className="mt-1 text-xs text-[var(--sdep-text-muted)]">Every route answers a specific question while the source ministry keeps custody of its records.</p></div><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{USE_CASES.map((useCase) => { const selected = useCase.id === activeUseCase.id; const Icon = useCase.icon; return <button key={useCase.id} type="button" onClick={() => onSelectUseCase(useCase)} className={`rounded-xl border p-4 text-left transition ${selected ? 'border-[var(--sdep-teal)] bg-[rgba(26,168,145,0.12)]' : 'border-[var(--sdep-border)] bg-[rgba(7,16,24,0.44)] hover:border-[var(--sdep-border-strong)]'}`}><div className="flex items-start justify-between"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--sdep-surface-raised)] text-[var(--sdep-teal)]"><Icon className="h-4 w-4" /></span>{selected && <span className="text-[10px] font-semibold uppercase tracking-wider text-[var(--sdep-teal)]">Active</span>}</div><h4 className="mt-3 text-sm font-semibold text-[var(--sdep-text)]">{useCase.name}</h4><p className="mt-1 text-xs text-[var(--sdep-text-muted)]">{useCase.requestingShort} <span className="mx-1 text-[var(--sdep-teal)]">to</span> {useCase.targetShort}</p><p className="mt-3 text-xs leading-relaxed text-[var(--sdep-text-subtle)]">{useCase.zkpValueDelivered}</p></button>; })}</div></div>;
}
...
$$;`;
  ...
  return <div className="space-y-4"><div><h3 className="text-base font-semibold text-[var(--sdep-text)]">Data protection controls</h3><p className="mt-1 text-xs text-[var(--sdep-text-muted)]">The requesting ministry receives the answer it needs, not the record used to calculate it.</p></div><div className="overflow-hidden rounded-xl border border-[var(--sdep-border)]"><div className="grid grid-cols-3 gap-3 bg-[var(--sdep-surface)] px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-[var(--sdep-text-subtle)]"><span>Data element</span><span>Handling</span><span>External access</span></div>{rows.map(([name, handling, access]) => <div key={name} className="grid grid-cols-3 gap-3 border-t border-[var(--sdep-border)] px-3 py-3 text-xs"><span className="font-semibold text-[var(--sdep-text)]">{name}</span><span className="text-[var(--sdep-text-muted)]">{handling}</span><span className="text-[var(--sdep-success)]">{access}</span></div>)}</div><div className="rounded-xl border border-[var(--sdep-border)] bg-[var(--sdep-ink)] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-semibold text-[var(--sdep-text)]">Public RPC wrapper</p><p className="mt-1 text-[11px] text-[var(--sdep-text-muted)]">The only externally callable surface returns proof fields, never raw vault columns.</p></div><button type="button" onClick={copySql} className="flex items-center gap-1.5 rounded-lg border border-[var(--sdep-border)] px-2.5 py-1.5 text-xs font-semibold text-[var(--sdep-blue)]">{copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}{copied ? 'Copied' : 'Copy SQL'}</button></div><pre className="mt-3 max-h-64 overflow-auto rounded-lg border border-[var(--sdep-border)] bg-[#050c12] p-3 font-mono text-[10px] leading-relaxed text-[var(--sdep-blue)]">{publicWrapperSql}</pre></div><button type="button" onClick={onOpenSchema} className="rounded-lg border border-[var(--sdep-border)] bg-[var(--sdep-surface)] px-3 py-2 text-xs font-semibold text-[var(--sdep-text-muted)] hover:border-[var(--sdep-border-strong)] hover:text-[var(--sdep-text)]">Open complete migration and RLS policy</button></div>;
}

function BriefingPanel({ onOpenPitch }: { onOpenPitch: () => void }) {
  return <div className="space-y-5"><div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-start"><div><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--sdep-teal)]">Executive summary</p><h3 className="mt-2 text-2xl font-semibold text-[var(--sdep-text)]">Verification without exposure.</h3><p className="mt-3 max-w-2xl text-sm leading-relaxed text-[var(--sdep-text-muted)]">SDEP lets one ministry verify a citizen&apos;s eligibility through a signed mathematical result while the data custodian retains ownership of the underlying record. The exchange is purpose-limited, auditable, and designed to minimize sensitive data movement.</p><div className="mt-5 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-[var(--sdep-border)] bg-[rgba(7,16,24,0.44)] p-3"><strong className="text-sm text-[var(--sdep-success)]">0 bytes</strong><p className="mt-1 text-xs text-[var(--sdep-text-muted)]">Protected records exposed</p></div><div className="rounded-xl border border-[var(--sdep-border)] bg-[rgba(7,16,24,0.44)] p-3"><strong className="text-sm text-[var(--sdep-blue)]">Signed</strong><p className="mt-1 text-xs text-[var(--sdep-text-muted)]">Proof returned with every answer</p></div><div className="rounded-xl border border-[var(--sdep-border)] bg-[rgba(7,16,24,0.44)] p-3"><strong className="text-sm text-[var(--sdep-warning)]">Audited</strong><p className="mt-1 text-xs text-[var(--sdep-text-muted)]">Exchange recorded for review</p></div></div></div><button type="button" onClick={onOpenPitch} className="rounded-lg bg-[var(--sdep-teal)] px-4 py-2.5 text-xs font-semibold text-[#061018] hover:bg-[#82e4d3]">Open full briefing</button></div><div className="rounded-xl border-l-2 border-[var(--sdep-teal)] bg-[var(--sdep-ink)] p-4 text-sm leading-relaxed text-[var(--sdep-text-muted)]"><p className="font-semibold text-[var(--sdep-text)]">Briefing narrative</p><p className="mt-2">When a ministry needs to verify eligibility, it does not need another ministry&apos;s database. It needs a simple answer. SDEP runs the logic where the data lives and returns a signed mathematical proof.</p><p className="mt-2">The requesting ministry gets its decision. The data custodian keeps ownership. No raw salary, medical, family, or criminal-history records leave the source vault.</p></div></div>;
}